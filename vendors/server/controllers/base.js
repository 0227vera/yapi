...
class baseController {
  ...
  async getLoginStatus(ctx) {
    ...
    if ((await this.checkLogin(ctx)) === true) {
      ...
      // 未登录都统一当做游客
      let guestEmail = 'guest@lezhixing.com.cn'
      let userInst = yapi.getInst(userModel); //创建user实体
      let guset = await userInst.findByEmail(guestEmail)
      let result = {
        username: guset.username,
        role: guset.role,
        uid: guset._id,
        _id: guset._id,
        email: guset.email,
        add_time: guset.add_time,
        up_time: guset.up_time,
        type: guset.type || 'third',
        study: guset.study
      }
      let token = jwt.sign({ uid: guset._id }, guset.passsalt, { expiresIn: '7 days' });
      ctx.cookies.set('_yapi_token', token, {
        expires: yapi.commons.expireDate(7),
        httpOnly: true
      })
      ctx.cookies.set('_yapi_uid', guset._id, {
        expires: yapi.commons.expireDate(7),
        httpOnly: true
      })
      body = yapi.commons.resReturn(result)
  ...
  async getProjectRole(id, type) {
    ...
    try {
      ...
      if (type === 'project') {
        ...
        // warnging: 不清楚什么情况下会继续变成group来获取项目权限，暂时去掉, 直接将未在project中的用户身份设置为member
        // type = 'group';
